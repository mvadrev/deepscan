...
unsigned long pill_count_update_time = 0;
const unsigned long pill_count_display_duration = 5000; // 5 seconds
...
void post_image_and_show_count() {
  ...
  // ✅ Convert RGB565 snapshot to JPEG before sending
  uint8_t *jpg_buf = NULL;
  size_t jpg_len = 0;

  camera_fb_t fb_fake;
  fb_fake.width = snapshot_img.header.w;
  fb_fake.height = snapshot_img.header.h;
  fb_fake.format = PIXFORMAT_RGB565;
  fb_fake.buf = (uint8_t *)snapshot_img.data;
  fb_fake.len = snapshot_img.data_size;

  bool jpeg_ok = frame2jpg(&fb_fake, 12, &jpg_buf, &jpg_len);

  if (jpeg_ok && jpg_buf != NULL && jpg_len > 0) {
    ...
    if (httpResponseCode == HTTP_CODE_OK) {
      ...
      if (!error && doc.containsKey("pill_count")) {
